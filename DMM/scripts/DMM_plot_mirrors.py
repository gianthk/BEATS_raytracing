...
# PLOT DMM MIRRORS          ##################################################################################
f, ax = plt.subplots()
plt.plot(M1[:,0], M1[:,1], 'k', linewidth=3)
plt.plot(M2[:,0], M2[:,1], 'k', linewidth=3)
plt.plot(M1[1, 0], M1[1, 1], 'Xr')
plt.plot(M2[1,0], M2[1,1], 'Xr')
plt.plot(ray_low[0,:], ray_low[1,:], 'c', linewidth=1)
plt.plot(ray_high[0,:], ray_high[1,:], 'c', linewidth=1)

plt.xlabel('Z [m]')
plt.ylabel('Y [m]')
plt.grid(True, which="both")
plt.xticks(np.arange(-0.4, 1, 0.1))
plt.yticks(np.arange(-0.005, 0.026, 0.001))
