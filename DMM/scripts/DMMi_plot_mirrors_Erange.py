...
dx = l+0.02 # [m] max distance between the two mirrors along X
dy_default = 0.062 # [m] min distance between the two mirrors along Y
...
dy = float(input("Insert min Y distance between mirrors [mm] (default: 6.2mm): "))*1e-3 # [mm]
# if dy.isnumeric():
#     dy = float(dy)*1e-3
# else:
#     dy = dy_default
...
# absolute distance between the mirrors
d = np.sqrt(dx**2 + dy**2)

# default min angle between mirror positions (at Bragg=0.. considering the beam height)
theta_0 = np.arcsin(dy/d)

# init array of offsets
o_array = np.zeros([len(theta_array), 1], 'float')

f, ax = plt.subplots()
exit_ray_x = dx+(l/2)+0.1
count = 0
for theta in theta_array:
    ...
    L_1_p = d*np.cos(theta_0+theta) # [m]

    # VERTICAL DISTANCE BETWEEN THE MIRRORS
    o_0_p = d * np.sin(theta_0 + theta)  # [m]
    o_array[count] = o_0_p
...
print(f"DMM OFFSET: {(o_array.min()*1e3):.3} - {(o_array.max()*1e3):.3} [mm]")
