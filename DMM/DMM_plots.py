...
d_spacing = np.array([2.5, 3.0, 4.0]) # [nm]
...
theta = np.arcsin(np.transpose(np.array([lambda_E, lambda_E, lambda_E]))/(2*d_spacing)) # [rad] working grazing angle
...
plt.legend(['d-spacing = 2.5nm', 'd-spacing = 3nm', 'd-spacing = 4nm'], loc='upper right')
plt.xticks(np.arange(5, 65, 5))
plt.yticks(np.arange(0.1, 2.7, 0.1))
ax.set(xlim=(9, 51), ylim=(0.1, 1.6))
plt.xlabel('Photon energy [keV]')
plt.ylabel('grazing angle [deg]')
plt.grid(True, which="both")
plt.show()
f.savefig(f"DMM_theta.png", bbox_inches='tight', dpi=600)
...
ax.set(xlim=(9, 51), ylim=(0.7, 9))
plt.xlabel('Photon energy [keV]')
plt.ylabel('DMM mirror1 height [mm] \n (mirror length = 300mm)')
plt.grid(True, which="both")
plt.show()
f.savefig(f"DMM_mirror1_height_L{l*1e3}mm.png", bbox_inches='tight', dpi=600)
...
plt.grid(True, which="both")
ax.set(xlim=(9, 51), ylim=(1, 22))
plt.yticks(np.arange(0, 24, 2))
f.savefig(f"DMM_beam_height_43m_L{l*1e3}mm.png", bbox_inches='tight', dpi=600)

# beam size and divergences from Shadow at different energies          #################

# f, ax = plt.subplots()
plt.plot(E2/1e3, 1e3*(FWHM_Y_MM1/d_MM1)*d_sample, '--m', linewidth=2)
plt.legend(['d-spacing = 2.5nm', 'd-spacing = 3nm', 'd-spacing = 4nm', 'without DMM (FWHM)'], loc='upper right')
plt.xticks(np.arange(5, 65, 5))
plt.yticks(np.arange(1, 19, 1))
ax.set(xlim=(8, 61), ylim=(1.4, 19))
plt.show()
# f.savefig(f"DMM_beam_height@43m_L{l*1e3}mm.png", bbox_inches='tight', dpi=600)
